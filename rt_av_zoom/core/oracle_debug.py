...
def main():
    ...
    if not os.path.exists(f"{OUTDIR}//target_reference.wav") or not os.path.exists(f"{OUTDIR}//interference_reference.wav"):
        print("Error: Reference files missing. Run world.py first.")
        return

    y_mix, _ = sf.read(f"{OUTDIR}//mixture.wav", dtype='float32') # [Samples, 2]
    y_mix = y_mix.T 
    
    s_tgt_ref, _ = sf.read(f"{OUTDIR}//target_reference.wav", dtype='float32')
    s_int_ref, _ = sf.read(f"{OUTDIR}//interference_reference.wav", dtype='float32')
    ...
    sf.write(f"{OUTDIR}/output_oracle.wav", s_out, FS)
    print(f"Saved '{OUTDIR}/output_oracle.wav'.")
