'''
python world.py --no-reverb --dataset ljspeech --n 2
'''
...
def main():
    ...
    parser.add_argument('--dataset', type=str, default='ljspeech', choices=['musan', 'librispeech', 'ljspeech'])
    ...
    # --- 3. Add Sources (Circular Placement) ---
    mic_center = np.mean(MIC_LOCS, axis=1) # [2.45, 2.45, 1.5]
    
    # Radius for uniform distance (2m fits safely in a 4.9m room)
    RADIUS = 2.0 
    
    # Minimum distance between any two sources (arc length approx)
    MIN_SRC_DIST = 0.5 
    
    # Keep track of all placed source positions to check against
    placed_sources = []

    # 1. Place Target (Fixed Angle or Random?)
    # Let's keep your original fixed logic but project it to the circle
    # Original POS_TARGET was [2.45, 3.45, 1.5]. Relative y=1, x=0 -> 90 degrees.
    # We will enforce the RADIUS explicitly.
    ...
    # 2. Place Interferers
    if args.n > 0:
        sources_to_add = args.n
        
        # (Optional) Retain the fixed 40-degree interferer if desired?
        # If yes, add it first.
        # 40 degrees = 40 * pi / 180 radians
        interf_angle_fixed = 40 * (np.pi / 180) 
        pos_fixed = [
            mic_center[0] + RADIUS * np.cos(interf_angle_fixed),
            mic_center[1] + RADIUS * np.sin(interf_angle_fixed),
            1.5
        ]
        
        # Check if fixed pos conflicts with target (it shouldn't, 90 vs 40 is far)
        if np.linalg.norm(np.array(pos_fixed) - np.array(pos_target)) > MIN_SRC_DIST:
             room.add_source(pos_fixed)
             placed_sources.append(np.array(pos_fixed))
             sources_to_add -= 1
        
        # 3. Place Remaining Random Interferers
        for i in range(sources_to_add):
            max_attempts = 100
            for attempt in range(max_attempts):
                # Pick random angle 0 to 2pi
                ...
                # Check distance against ALL previously placed sources
                collision = False
                for existing_src in placed_sources:
                    dist = np.linalg.norm(candidate - existing_src)
                ...
                print(f"Warning: Could not place Interferer {i+2} (Space too crowded!)")
    ...
    # --- 6. SIR Control (DISABLED for Natural Physics) ---
    # We calculate stats just for logging, but we do NOT apply gain.
    p_target = np.mean(target_ch1 ** 2)
    p_interf = np.mean(interf_ch1_total ** 2)
    
    # Calculate what the natural SIR turned out to be
    if p_interf > 0:
        natural_sir = 10 * np.log10(p_target / p_interf)
        print(f"Natural SIR (Physics-based): {natural_sir:.2f} dB")
    
    # Force gain to 1.0 (Do not scale interferers)
    gain = 1.0 
    ...
    final_ch2, noise_ch2 = add_awgn(clean_mix_ch2, SNR_TARGET_DB+3)


    # print(final_ch1 - final_ch2)
    # matrix = np.stack([clean_mix_ch1, clean_mix_ch2])
    # noise_corr = np.stack([noise_ch1, noise_ch2])
    ...
    # print(f"Correlation between Ch1 and Ch2: {correlation:.4f}")
    ...
    # Save Files
    sf.write("estimator/mixture.wav", stereo_mix, FS)
