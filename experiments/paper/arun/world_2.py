...
def mix_and_save(files, prefix):
    ...
    sf.write(f"mixture_{prefix}_4.wav", mix / norm, FS)
    sf.write(f"target_ref_{prefix}_4.wav", tgt_ref / norm, FS)
    sf.write(f"interf_ref_{prefix}_4.wav", int_ref / norm, FS)
    print(f"Created mixture_{prefix}_4.wav with 4 sources")
...
def main():
    ...
    if not files or len(files) < 8: # Check for enough files
        print("Error: Data not found or insufficient files (need at least 8).")
        return

    random.shuffle(files)
    
    # [CHANGE 3] Slice 4 files instead of 3 for the mix
    mix_and_save(files[:4], "TRAIN")
    mix_and_save(files[4:8], "TEST")
