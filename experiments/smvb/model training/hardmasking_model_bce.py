...
DROPOUT_RATE = 0.3  # <--- NEW: Dropout Probability

# --- HARD MASK SETTINGS ---
MASK_THRESHOLD = 0.5

# --- 1. Audio Saving Helper ---
def save_debug_audio(raw_mix_stft, raw_target_stft, true_mask, pred_mask, epoch, save_dir="debug_audio"):
    ...
    fs = CONF["fs"]; n_fft = CONF["n_fft"]; hop = CONF["hop_len"]
    ...
    M_pred_binary = (M_pred > MASK_THRESHOLD).astype(float)
   
    Z_masked_true = Z_mix * M_true
    sf.write(f"{epoch_dir}/mask_truth_audio.wav", istft_stereo(Z_masked_true), fs)
   
    Z_masked_pred = Z_mix * M_pred_binary
    sf.write(f"{epoch_dir}/mask_pred_binary_audio.wav", istft_stereo(Z_masked_pred), fs)
...
def visualize_masks(input_feat, true_mask, pred_mask, epoch):
    spec = input_feat[0, 0].detach().cpu().numpy()
    ...
    plt.subplot(1, 3, 2)
    plt.title("True Hard Mask")
    plt.imshow(truth, aspect='auto', origin='lower', cmap='gray', vmin=0, vmax=1)
    plt.colorbar()
...
class PhysicsAudioDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        St_padded = pad_tensor(S_t[np.newaxis, ...])
        ...
        # Hard Mask
        irm_soft = mag_t / (mag_t + mag_i + 1e-7)
        binary_mask = (irm_soft > MASK_THRESHOLD).astype(np.float32)
...
# --- 5. UPDATED Model Architecture with DROPOUT ---
class ResBlock(nn.Module):
    def __init__(self, channels, dropout=0.0):
        ...
        self.conv = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Dropout2d(p=dropout), # <--- Added Dropout
            nn.Conv2d(channels, channels, 3, padding=1),
            nn.BatchNorm2d(channels)
        )
        self.relu = nn.ReLU()
...
class DeepFPU(nn.Module):
    def __init__(self, dropout=DROPOUT_RATE):
        ...
        # Add dropout to blocks
        self.enc1_conv = nn.Sequential(
            nn.Conv2d(5, 32, 3, padding=1), nn.BatchNorm2d(32), nn.ReLU(), nn.Dropout2d(p=dropout),
            nn.Conv2d(32, 32, 3, padding=1), nn.BatchNorm2d(32), nn.ReLU(), nn.Dropout2d(p=dropout)
        )
        self.enc2_conv = nn.Sequential(nn.Conv2d(32, 64, 3, padding=1), nn.BatchNorm2d(64), nn.ReLU(), nn.Dropout2d(p=dropout), ResBlock(64, dropout))
        self.enc3_conv = nn.Sequential(nn.Conv2d(64, 128, 3, padding=1), nn.BatchNorm2d(128), nn.ReLU(), nn.Dropout2d(p=dropout), ResBlock(128, dropout))
        self.enc4_conv = nn.Sequential(nn.Conv2d(128, 256, 3, padding=1), nn.BatchNorm2d(256), nn.ReLU(), nn.Dropout2d(p=dropout), ResBlock(256, dropout))
        self.bottleneck = nn.Sequential(nn.Conv2d(256, 512, 3, padding=1), nn.BatchNorm2d(512), nn.ReLU(), nn.Dropout2d(p=dropout), ResBlock(512, dropout), ResBlock(512, dropout))
       
        self.up4 = nn.ConvTranspose2d(512, 256, (1, 2), stride=(1, 2))
        self.dec4_conv = nn.Sequential(nn.Conv2d(512, 256, 3, padding=1), nn.BatchNorm2d(256), nn.ReLU(), nn.Dropout2d(p=dropout), ResBlock(256, dropout))
       
        self.up3 = nn.ConvTranspose2d(256, 128, (1, 2), stride=(1, 2))
        self.dec3_conv = nn.Sequential(nn.Conv2d(256, 128, 3, padding=1), nn.BatchNorm2d(128), nn.ReLU(), nn.Dropout2d(p=dropout), ResBlock(128, dropout))
       
        self.up2 = nn.ConvTranspose2d(128, 64, (1, 2), stride=(1, 2))
        self.dec2_conv = nn.Sequential(nn.Conv2d(128, 64, 3, padding=1), nn.BatchNorm2d(64), nn.ReLU(), nn.Dropout2d(p=dropout), ResBlock(64, dropout))
       
        self.up1 = nn.ConvTranspose2d(64, 32, (1, 2), stride=(1, 2))
        self.dec1_conv = nn.Sequential(nn.Conv2d(64, 32, 3, padding=1), nn.BatchNorm2d(32), nn.ReLU(), nn.Dropout2d(p=dropout), nn.Conv2d(32, 32, 3, padding=1), nn.BatchNorm2d(32), nn.ReLU())
...
def main():
    ...
    model = DeepFPU(dropout=DROPOUT_RATE).to(DEVICE) # Pass dropout rate here
